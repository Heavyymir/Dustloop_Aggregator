...
// Parser to extract move framedata and descriptions from Marvel Tokon Character Pages
func ParseMTFS(data []byte) ([]models.Move, error) {
	...
	doc.Find(".attack-container").Each(func(_ int, container *goquery.Selection) {
		...
		// Extract technical and simple move inputs
		badgeContainer := heading.Find(".inputBadgeContainer")
		badgeSelector := ".inputBadge, .input-badge"

		techBadge := badgeContainer.Find(".toggle-group-a").Find(badgeSelector)
		simpleBadge := badgeContainer.Find(".toggle-group-b").Find(badgeSelector)
		...
		} else { // Fallback for normals or moves without toggle groups	
			fallBackBadge := badgeContainer.Find(badgeSelector)
			if fallBackBadge.Length() > 0 {
				move.Input = strings.Join(strings.Fields(fallBackBadge.Text()), " ")
			}	
		}

		if move.Name == "" || strings.Contains(move.Name, "Normals") || strings.HasSuffix(move.Name, "Attacks") {
		...
		// Extract Bullet Points/move notes
