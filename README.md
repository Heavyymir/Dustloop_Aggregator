...
Program Commands: 
	...
	• fetch: Used after select, fetches a character page and saves to SQLite DB. Usage: fetch <character> [-v|verbose] [-d|details]
...
When using Fetch:
If attempting to fetch a character that uses hyphens (Zato-1, Jack-o, I-no for Guilty Gear as examples) ensure that you are including the hyphen "-" to ensure that the
